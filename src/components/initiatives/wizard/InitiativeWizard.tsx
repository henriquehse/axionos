...
export function InitiativeWizard({ onSubmit, isPending }: Props) {
  ...
  const handleAnalyze = async (idea: string, referenceUrl?: string, competitor?: string, additionalContext?: string, depth?: string) => {
    ...
    try {
      let data, error;
      const isDemo = import.meta.env.VITE_SUPABASE_URL?.includes("dummy");
      
      if (isDemo) {
        // Mocking AI analysis for Demo Mode
        await new Promise(r => setTimeout(r, 2000));
        data = {
          blueprint: {
            project_name: idea.split(' ').slice(0, 3).join(' ') || "New Initiative",
            short_description: idea,
            problem_statement: "High cost of acquisition and low brand authority in the digital space.",
            target_audience: "Small business owners and marketing directors.",
            market_opportunity: "Growing demand for specialized performance marketing agencies.",
            competitor_insights: "Most agencies focus on volume. Opportunity lies in conversion-centric high-ticket landing pages.",
            suggested_features: ["Sales conversion funnel", "Lead qualification dashboard", "Performance metrics (PowerBI sync)"],
            suggested_integrations: ["Supabase", "Vercel", "Google Ads API", "Stripe"],
            product_type: "saas",
            estimated_complexity: "moderate",
            recommended_depth: "full_pipeline",
            reasoning: "The value proposition requires a robust lead management system and aesthetic depth to command high ticket prices."
          },
          meta: { cost_usd: 0.05, duration_ms: 2200 }
        };
        error = null;
      } else {
        const result = await supabase.functions.invoke("generate-initiative-blueprint", {
  ...
  const handleRunSimulation = async () => {
    ...
    try {
      ...
      if (isDemo) {
        ...
        data = {
          simulation_report: {
            blueprint_name: brief.name,
            viability_score: 94,
            infrastructure_ready: true,
            pipeline_recommendation: "Accelerated Wave 1 - Direct to Vercel Edge",
            risk_flags: [
              { type: "cost", severity: "low", message: "Initial cloud costs within threshold." },
              { type: "security", severity: "medium", message: "Standard auth patterns applied." }
            ],
            estimated_cost_range: { min_usd: 15.5, max_usd: 45.0 },
            estimated_time_minutes: { min: 5, max: 12 },
            agent_deployment_plan: [
              { agent: "Architect", task: "Database schema locking" },
              { agent: "Frontend Swarm", task: "Atomic component generation" }
            ],
            recommended_generation_depth: "full_pipeline"
          }
        };
        error = null;
      } else {
        const result = await supabase.functions.invoke("initiative-simulation-engine", {
          body: {
            initiative_id: "preview",
